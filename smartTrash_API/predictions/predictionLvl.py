...
def next_level(next_real_level = None):
    if model is None or scaler is None:
        raise RuntimeError("LSTM model or scaler not loaded.")
    ...
    # Without collection, level can only increase
    if next_real_level is not None:
        prediction = max(prediction, float(next_real_level))
    return prediction

def predict_weekly(current_level):
    """Predict fill levels for the next 7 days assuming no collection (levels only increase)."""
    if model is None or scaler is None:
        raise RuntimeError("LSTM model or scaler not loaded.")
    # Build a stable history from current_level instead of using the global deque
    # which may contain downward trends from past emptying events
    current_height = scaler.inverse_transform(
        np.array(current_level / 100.0).reshape(-1, 1)
    )[0][0]
    temp_heights = deque([current_height] * n_input, maxlen=n_input)

    daily_predictions = []
    prev_level = current_level
    for _ in range(7):
        pred = predict_next_height(list(temp_heights), model, scaler)
        pred_clamped = max(0.0, min(100.0, pred))
        # Enforce non-decreasing: bins fill up, they don't empty by themselves
        pred_clamped = max(pred_clamped, prev_level)
        daily_predictions.append(pred_clamped)
        prev_level = pred_clamped
        pred_height = scaler.inverse_transform(
            np.array(pred_clamped / 100.0).reshape(-1, 1)
        )[0][0]
        temp_heights.append(pred_height)
