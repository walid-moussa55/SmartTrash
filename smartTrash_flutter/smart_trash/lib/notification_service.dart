...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  DebugLogger.addDebugMessage("Handling a background message: ${message.messageId}");
...
class NotificationService {
  ...
  // Callback pattern (replaces GlobalKey anti-pattern)
  void Function(String binId)? onBinSelected;
  void Function()? onRefreshUI;
  AppUser? Function()? getCurrentUser;
  ...
  Future<void> initialize() async {
    ...
    if (!kIsWeb) {
      await subscribeToTopic(trashAlertsTopic);
      DebugLogger.addDebugMessage(
        "Subscribed to topic: $trashAlertsTopic",
      );
      // Optional: Subscribe to additional topics if needed
      await subscribeToTopic(trashAlertsTopicGas);
      DebugLogger.addDebugMessage(
        "Subscribed to topic: $trashAlertsTopicGas",
      );
      await subscribeToTopic(trashAlertsTopicEmergency);
      DebugLogger.addDebugMessage(
        "Subscribed to topic: $trashAlertsTopicEmergency",
      );
    } else {
  ...
  Future<void> _initializeWebNotifications() async {
    if (kIsWeb) {
      try{
        // TODO: Move VAPID key to environment config (.env) for production
        const vapidKey = 'BJhtHl-fqJmuvbvEgcXtcwMJ1FCaKVhoqUeeHqei1UCIlo0jlL2hQrDxAk94rY7JXfzleAbWg4QQc2xuCpKD9Uo';
  ...
  void _setupMessageHandlers() {
    // --- Handler for Foreground Messages (native only) ---
    // On web, foreground messages are handled in _initializeWebNotifications
    if (!kIsWeb) {
      FirebaseMessaging.onMessage.listen((RemoteMessage message) {
        DebugLogger.addDebugMessage("FCM Message Data: ${message.data}");
        DebugLogger.addDebugMessage("FCM Notification: ${message.notification?.title}");
        DebugLogger.addDebugMessage(
      ...
      switch (screen) {
        case 'map':
          if (data.containsKey('binId')) {
            onBinSelected?.call(data['binId'].toString());
          }
          break;
        case 'settings':
          // Settings navigation is handled by the HomeScreen callbacks
          DebugLogger.addDebugMessage('Navigation to settings requested via notification.');
          break;
        default:
