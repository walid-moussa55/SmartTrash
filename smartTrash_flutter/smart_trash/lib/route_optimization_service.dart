...
import 'package:naqi_ai/home_screen.dart' show TrashBin, Location; // For TrashBin, Location
...
class RouteOptimizationService {
  ...
  Future<OptimizedRouteResponse?> getOptimizedRoute({
    ...
    try {
      ...
      List<ApiBin> apiBins = allBins
          .where((bin) => bin.trashLevel >= 80)
          .map((bin) {
        return ApiBin(
          name: bin.name,
          location: bin.location,
          capacity: bin.trashLevel,
          volume: bin.volume ?? 100.0,
          weight: bin.weight,
        );
      }).toList();
      ...
      if (apiBins.isEmpty) {
        DebugLogger.addDebugMessage("No bins above 80% fill level for route optimization.");
        throw Exception("No bins above 80% fill level need collection.");
      }
